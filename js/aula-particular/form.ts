...
export function initContactForm(): void {
  ...
  const modalitySelect = form.querySelector<HTMLSelectElement>('#contactModality')
  ...
  // Enable submit only when name, modality, service and message are filled
  ...
  modalitySelect?.addEventListener('change', updateSubmitState)
  ...
  form.addEventListener('submit', (e: Event) => {
    ...
    const text = `Olá, Eduardo! Como você está? Me chamo *${name}*. Tenho interesse na aula de *${service || 'informática'}* — modalidade: *${modality || 'a definir'}*.${extra}`
